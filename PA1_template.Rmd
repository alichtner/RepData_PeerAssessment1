...
The maximum average 5-min interval occurs at **`r maxInt`**. This can be attributed to the morning rush hour commute.

## Creating Tidy Data

The total number of `NA` values in the raw data is **`r sum(is.na(raw$steps))`** and is found via `sum(is.na(raw$steps))`.
